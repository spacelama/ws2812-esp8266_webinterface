...
public:
    ...
    bool tripleClick(); // will be refreshed by poll()

    unsigned long _switchedTime, pushedTime, doubleClickedTime, tripleClickedTime;

protected:
    const byte pin;
    const int debounceDelay, longPressDelay, doubleClickDelay;
    const bool polarity;
    bool level, _switched, _longPress, longPressLatch, _doubleClick, _tripleClick;
};
